...
def self_heal_mode():
    ...
    codex_push = os.path.join(HOOKS_DIR, "codex_push.py")
    if not os.path.exists(codex_push):
        try:
            urllib.request.urlretrieve(f"{BASE_URL}/codex_push.py", codex_push)
            os.chmod(codex_push, 0o755)
            repaired.append("codex_push.py 재다운로드")
        except Exception:
            pass
